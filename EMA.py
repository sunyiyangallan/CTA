...
fm.at[0,'ema_short'] = fm.at[0,'CLOSE'] #第一个ema值设置为开始价格
fm.at[0,'ema_long'] = fm.at[0,'CLOSE'] #第一个ema值设置为开始价格

for i in range(1,fm.shape[0]):
    
    alpha1 = 2/(ma_short+1)
    alpha2 = 2/(ma_long+1)
    fm.at[i,'ema_short'] = alpha1 * fm.at[i,'CLOSE'] + (1-alpha1) * fm.at[i-1,'ema_short']
    fm.at[i,'ema_long'] = alpha2 * fm.at[i,'CLOSE'] + (1-alpha2) * fm.at[i-1,'ema_long']
...
condition1 = (fm['ema_short'] > fm['ema_long'])
condition2 = (fm['ema_short'].shift(1) < fm['ema_long'].shift(1))

fm.loc[condition1 & condition2,'signal'] = 1  #买入信号

condition3 = (fm['ema_short'] < fm['ema_long'])
condition4 = (fm['ema_short'].shift(1) > fm['ema_long'].shift(1))
